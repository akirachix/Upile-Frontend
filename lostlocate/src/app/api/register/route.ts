...
export async function POST(request: NextRequest) {
  ...
  try {
    const { first_name, last_name, role, phone_number, email, username } = await request.json();
    ...
    const response = await fetch(`${baseUrl}/users/register/`, {
      ...
      body: JSON.stringify({ first_name, last_name, role, phone_number, email, username }),
    });
